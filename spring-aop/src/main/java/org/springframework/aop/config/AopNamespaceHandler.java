...
public class AopNamespaceHandler extends NamespaceHandlerSupport {
	...
	@Override
	public void init() {
		// <aop:config> XML方式使用 AspectJAwareAdvisorAutoProxyCreator 创建代理
		registerBeanDefinitionParser("config", new ConfigBeanDefinitionParser());

		// <aop:aspectj-autoproxy>注解方式使用 AnnotationAwareAspectJAutoProxyCreator 创建代理
		registerBeanDefinitionParser("aspectj-autoproxy", new AspectJAutoProxyBeanDefinitionParser());

		// <aop:scoped-proxy/> 有scope属性的，表示bean的生存周期。scope的值有prototype、singleton、session、request
